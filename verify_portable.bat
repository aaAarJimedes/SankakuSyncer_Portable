...
set "CHECK_MODE="
if /i "%~1"=="--release" set "CHECK_MODE=--release"
if not "%~1"=="" if not defined CHECK_MODE (
...
"%~dp0Runtime\python.exe" -s -c "import os,runpy,sys;p=os.environ['SANKAKU_APP_DIR'];sys.path.insert(0,p);runpy.run_module('tools.portable_self_check',run_name='__main__')" %CHECK_MODE%
exit /b %ERRORLEVEL%
