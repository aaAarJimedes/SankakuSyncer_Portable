...
- 设置、任务篮与凭据事务的三把独立短时锁现在共用句柄绑定边界：从已绑定的普通 `Data/` 目录对象相对打开锁名，拒绝符号链接、Windows 重解析点、目录/FIFO、跨卷对象和多链接普通文件，且不读取、截断或改写既有锁内容。Windows 通过 `NtCreateFile(RootDirectory=...)` 和不共享写入/删除把锁名固定到整个临界区；POSIX 使用 `openat`/`O_NOFOLLOW`、非阻塞打开与 `flock`，并在加锁前后和正常退出前复核目录项与 fd 身份。独立子进程争用、链接换向、三锁嵌套、正文异常与退出复核均有离线回归，Linux CI 直接执行 POSIX 专测；标签发布还会在最终 assembled 便携树内先跑完整回归，再生成清单与 ZIP。
- 任务篮在临近原子替换前再次核对完整 SHA-256；在这次最终复核前检测到外部程序于临时文件写盘期间放入的新版本时会冲突保留。只有完整读取、散列并在任务锁内再次匹配的损坏文件才会隔离；超过 16 MiB、未读取正文的文件保持原位并中止启动。
- 设置文件以稳定有界快照和 `(字节数, SHA-256)` 识别磁盘版本，独立短时跨进程锁覆盖保存前重检、临近替换复核、原子替换和写后确认；陈旧窗口、外部删除及保持相同大小和时间戳的改写都会冲突退出，不再静默覆盖较新的普通设置或凭据标记。
- 无法可靠读取、持续变化或超过 1 MiB 的设置保持原位并进入只读；只有已完整散列且再次核对摘要的损坏文件才会隔离。关闭窗口时若检测到外部更新，会提示窗口状态未保存并保持外部文件不变。
- 凭据启用或禁用与外部设置更新冲突时，无秘密事务 journal 和 vault receipt 继续阻止旧会话回退；新实例恢复事务后会保留外部普通设置，并只完成匹配的新会话或禁用状态。
- API JSON、媒体下载和远程缩略图共用严格有界的 `Content-Length` 规则；只有字段真正缺失时才允许未知长度，空值、符号、空白、非 ASCII 数字、超长数值以及重复字段都会失败关闭，不会被当成“未声明”或静默采用最后一个值。
- 已声明长度必须与本次实际接收字节完全一致：超长正文会立即停止，短读会在 EOF 拒绝。可重放 GET 仍按既有预算重试，密码或变更 POST 仍绝不重放；缩略图不会发出部分成功，Range 续传同时保持完整资源位置校验。
- `sample`、`preview` 和远程缩略图必须同时具备受支持的图片签名与物理结束边界；三字节 JPEG 文件头、缺少 EOI/IEND/Trailer、WebP RIFF 长度不符，或使物理文件不再以相应终止边界结束的尾随字节都会失败关闭，不再保存或发出缩略图成功。
- 这项检查是流式可用的必要容器边界门槛，不声称完成像素解码或验证文件中部；普通与绑定根目录的本地复核从同一次 SHA-256/MD5 读取取得固定长度文件尾，不重开路径且保持固定内存开销。
- 下载成功或失败后的精确任务终态会保留并在批次结束时原样重试；一次性写盘故障不再丢失输出路径或原始错误，持续故障也不会被误报为已经恢复。
- 超长或非 ASCII 的媒体长度响应头会在整数转换前安全拒绝并显示明确协议错误；异常续传范围不会破坏已有 `.part` 和状态文件。
- 便携组装和确定性 ZIP 在规范化路径前拒绝源码或 Runtime 根目录链接及 Windows 重解析点，保持发布输入必须为普通目录的边界。
- API、媒体下载与缩略图统一解析 `Retry-After` 和限流重置时间；非有限、负数或溢出响应不会绕过冷却或造成 24 小时误延期，而会继续采用有效备用头或保守默认值。
- 设置文件若因权限或共享冲突暂时无法读取，会保持原文件和 DPAPI 凭据不变；本次启动禁用设置、凭据以及依赖推测下载目录的写入操作，重启读取成功后恢复。确认损坏的内容仍先原样备份，极端溢出数值也不会中断启动。
- 本地下载库在搜索、筛选、排序和报告刷新后保留仍可见的多选、焦点列、视口位置和离线预览；同路径内容变化或下载根目录切换会安全重建预览，隐藏项不会在后续刷新中重新选中。
