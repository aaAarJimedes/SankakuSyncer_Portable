...
- 登录：用户名和密码只发往 `https://sankakuapi.com/auth/token`；令牌与密码不进入普通设置、日志或命令行。本机保存时用 Windows DPAPI CurrentUser 加密。
...
- 保守失败：401/403 停止并提示；429 遵循 `Retry-After`/`X-RateLimit-Reset`，无信号默认等待 10 分钟；5xx 有界退避；不自动切换旧接口。
