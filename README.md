...
- 新增“本地下载库”：仅在已保存下载目录第一层做只读离线扫描，重新校验 sidecar、SHA-256、媒体签名和文件身份。
- 结果可按“已验证、内容变化、元数据无效、缺媒体、缺元数据、路径不安全、无法读取”筛选；取消或失败不会覆盖上次完整报告。
...
5. 在“本地下载库”中手动扫描，查看哈希、签名或 sidecar 异常；此页不会删除、修复或自动重下。
