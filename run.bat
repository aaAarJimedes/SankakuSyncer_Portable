...
title SankakuSyncer
cd /d "%~dp0"
rem Drop inherited Chromium debugging/sandbox/proxy state before Qt is loaded.
set "ALL_PROXY="
set "HTTP_PROXY="
set "HTTPS_PROXY="
set "NO_PROXY="
...
start "" "%~dp0Runtime\pythonw.exe" -s -c "import os,runpy,sys;p=os.environ['SANKAKU_APP_DIR'];sys.path.insert(0,p);runpy.run_path(os.path.join(p,'main.py'),run_name='__main__')"
